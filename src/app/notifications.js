...
export default class VueNotifications {
    ...
    async disablePushNotifications ( user = true ) {
        ...
        try {
            if ( !firebase.isSupported ) throw result( [ 500, l10n( "Push notifications are not supported" ) ] );

            const disabled = await firebase.disable();

            if ( !disabled ) throw result( [ 500, l10n( "Error disabling push notifications" ) ] );
    ...
    async #enablePushNotifications () {
        if ( !this.pushNotificationsSupported ) return result( [ 500, l10n( "Push notifications are not supported" ) ] );
        ...
        if ( !token ) {
            ...
            return result( [ 500, l10n( "Push notifications are disabled in the browser settings" ) ] );
        }
